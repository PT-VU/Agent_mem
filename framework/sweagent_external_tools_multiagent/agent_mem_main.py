...
# T1-B: InterimCache for async interim card writes
try:
    from .agent_mem.storage.interim_cache import InterimCache as _InterimCache
except ImportError:
    try:
        from agent_mem.storage.interim_cache import InterimCache as _InterimCache
    except ImportError:
        _InterimCache = None  # type: ignore[assignment,misc]
...
def handle_interim_card_write(event_data: Dict[str, Any]) -> Dict[str, Any]:
    """T1-B: Handle interim_card_write event from bridge_hook async thread."""
    if _InterimCache is None:
        return {"status": "skipped", "reason": "InterimCache not available"}
    try:
        cache = _InterimCache.from_env()
        ok = cache.write_interim_card(
            instance_id=event_data.get("instance_id", ""),
            attempt_id=event_data.get("attempt_id", ""),
            card_type=event_data.get("card_type", "InterimLocalizationCard"),
            localization=event_data.get("localization", {}),
            source_step=int(event_data.get("source_step", 0)),
        )
        return {"status": "ok" if ok else "error", "event": "interim_card_write"}
...
def handle_event(adapter: SWEAgentAdapter, event_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elif event_type == "interim_card_write":
        # T1-B: async interim localization write (does not require adapter)
        return handle_interim_card_write(event_data)
    else:
        return {
            "error": f"Unsupported event type: {event_type}",
            "supported_events": [
                "plan_generated", "action_error", "run_done",
                "official_eval_feedback", "interim_card_write",
            ],
