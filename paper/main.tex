...
Coverage improves from 75.0\% to 100.0\% at \texttt{resolved@10}, but the average first resolved attempt becomes later (1.7 to 3.2) and post-success consistency decreases slightly (52.0\% to 48.1\%). T1-A fires 251 times across 40 attempts, whereas T1-C produces no verdicts because ordinary \texttt{submit} actions lack inline patch text. Follow-up checks make T1-C observable, but its decisions remain dominated by \texttt{approve} or \texttt{unavailable}. We therefore interpret this study as preliminary evidence of coverage amplification, not as a mature reliability improvement. The SymPy failures further show that a pre-submit critic cannot address exploration runs that never produce a candidate patch.
